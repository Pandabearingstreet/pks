delineate_conjunctive_skillfun_only <- function(skillfun, itemID = 1, verbose = FALSE) {
...
delineate_disjunctive_skillfun_only <- function(skillfun, itemID = 1, verbose = FALSE) {
  skillfun <- make_dis_to_con(skillfun)
  states <- delineate_conjunctive_skillfun_only(skillfun, itemID, verbose = verbose)
  states <- +(!states)
  return(states)
}

delineate.by.cheating <- function(skillfun, itemID = 1, verbose = FALSE) {
  ...
  if (isDis(skillfun, itemID)) {
    states <- delineate_disjunctive_skillfun_only(skillfun, itemID, verbose = verbose)
    rownames(states) <- concat_rows(states)
    return(states)
  }
  
  new_sf <- rename_duplicates(skillfun, itemID)
  bloated_states <- delineate_conjunctive_skillfun_only(new_sf, itemID, verbose = verbose)
